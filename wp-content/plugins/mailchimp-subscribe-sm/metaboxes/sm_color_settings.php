...
function ssm_color_settings_meta($post){
    ...
    if (empty($smf_body_bg)) {
        $smf_body_bg = '#fff';
    }
    if (empty($smf_form_bg)) {
        $smf_form_bg = '#fff';
    }
    if (empty($smf_cta_bg)) {
        $smf_cta_bg = '#dd541c';
    }
    if (empty($smf_content_c)) {
        $smf_content_c = '#424242';
    }
    if (empty($smf_cta_c)) {
        $smf_cta_c = '#fff';
    }
...
<h2>Background Colors</h2>
    ...
    <label for='smf_body_bg'>Body Background Color : </label>
     <input type='text' class='color_picker' name='smf_body_bg' data-alpha="true" value='<?php echo $smf_body_bg; ?>'/>
     <br>
     <br>

	<label for='smf_form_bg'>Form Background Color : </label>
	 <input type='text' class='color_picker' name='smf_form_bg' data-alpha="true" value='<?php echo $smf_form_bg; ?>'/>
	 <br>
 	 <br>
 	 <label for='smf_cta_bg'>Button Background Color : </label>
	 <input type='text' class='color_picker' name='smf_cta_bg' data-alpha="true" value='<?php echo $smf_cta_bg; ?>'/>
	 <br>
 	 <br>
	 ...
	 <input type='text' class='color_picker' name='smf_content_c' data-alpha="true" value='<?php echo $smf_content_c; ?>'/>
	 <br>
 	 <br>
	
	 <label for='smf_cta_c'>Button Text Color : </label>
	 <input type='text' class='color_picker' name='smf_cta_c' data-alpha="true" value='<?php echo $smf_cta_c; ?>'/>
	 <br>
 	 <br>
