...
class Alg_WC_Currency_Switcher_Settings_Flags extends Alg_WC_Currency_Switcher_Settings_Section {
	...
	function get_flags_settings( $settings ) {
		...
		$show_flags_in_settings = ( 'yes' === get_option( 'alg_currency_switcher_show_flags_in_admin_settings_enabled', 'yes' ) );
			...
			if ( '' != $currency ) {
				...
				$settings = array_merge( $settings, array(
					array(
						...
						'class'    => ( $show_flags_in_settings ? 'alg-wselect' : '' ),
						'css'      => ( $show_flags_in_settings ? 'background-color: transparent; box-shadow: 0 0 2px transparent;' : '' ),
					),
				) );
			}
		}
